...
/**
 * Servlet implementation class InsertTravel
 */
...
public class InsertTravel extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Date date = null;
		try {
			date = simpleDateFormat.parse(request.getParameter("startDate"));
		...
		boolean bl = new TravelDaoImp().add(travels);
		if (bl) {
			response.sendRedirect("admin/travel/index.jsp");
			
		}else {
			request.getRequestDispatcher("admin/travel/insertTravel.jsp").forward(request, response);
		}
	}
